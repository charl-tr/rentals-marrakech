...
- Mémoire d'envoi des formulaires (`form-memory`), appliquée au formulaire de contact **et** au formulaire de dépôt vendeur : au refresh, le formulaire ne réapparaît plus vierge — il reconnaît « demande déjà envoyée » (contact : avec le conseiller assigné ; dépôt : « c'est noté, on vous rappelle »), clé par contexte (bien précis ou générique), avec réouverture explicite (« Envoyer un autre message » / « Déposer un autre bien »). Supprime l'impression de non-envoi et la re-soumission accidentelle.
